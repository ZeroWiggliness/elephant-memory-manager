...
namespace Elephant
{
	...
	void *MemoryManagerDefaultSystemAllocator(jrs_u64 uSize, void *pExtMemoryPtr)
	{
		return mmap(pExtMemoryPtr, uSize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	}
	...
	void MemoryManagerPlatformInit(void)
	{
		// Place any specific network intialization here.
	...
	void MemoryManagerPlatformFunctionNameFromAddress(jrs_sizet uAddress, jrs_i8 *pName, jrs_sizet *pFuncStartAdd, jrs_sizet *pFuncSize)
	{
        ...
        if(dladdr((void *)uAddress, &DlInfo) != 0)
        {
            jrs_i8 name[2048];
            ...
            if(strlen(name) > 127)
                name[127] = 0;
            strcpy(pName, name);
            *pFuncStartAdd = (jrs_sizet)DlInfo.dli_saddr;
            *pFuncSize = (uAddress - (jrs_sizet)DlInfo.dli_saddr) + 1;
        }
	}
	...
	void cMemoryManager::StackTrace(jrs_sizet *pCallStack, jrs_u32 uCallstackDepth, jrs_u32 uCallStackCount)
	{
        ...
        jrs_i32 frames = backtrace(callstack, 32);
        for(jrs_i32 i = uCallstackDepth; i < uCallStackCount && i < frames; i++)
        {
            pCallStack[i - uCallstackDepth] = (jrs_sizet)callstack[i]; 
        }
	}
	...
	void cPoolBase::StackTrace(jrs_sizet *pCallStack)
	{
        ...
        jrs_i32 frames = backtrace(callstack, 32);
        for(jrs_i32 i = 3; i < JRSMEMORY_CALLSTACKDEPTH && i < frames; i++)
        {
            pCallStack[i - 3] = (jrs_sizet)callstack[i]; 
        }
	}
	...
	jrs_i32 SendSocket(jrs_socket s, const jrs_i8 *buf, jrs_i32 len)
	{
		return (jrs_i32)send((SOCKET)s, buf, len, 0);
	}
	...
	jrs_i32 RecvSocket(jrs_socket s, jrs_i8 *buf, jrs_i32 len)
	{
		return (jrs_i32)recv((SOCKET)s, buf, len, 0);
	}
}